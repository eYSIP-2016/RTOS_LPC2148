...
void Txtask1(void *p)			// task which writes data on to the Queue
{
	while(1)
	{
					  
		if(xQueueSend(xQueue,tx1,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
		{  //vTaskResume("RxTask");
		//Data added to Q
				}				  
		else
...
void Txtask2(void *p)			// task which writes data on to the Queue
{
 
	while(1)
	{
					  
		if(xQueueSend(xQueue,tx2,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
		{ // vTaskResume("RxTask");
		 //Data added to Q	 
		}				  
		else
...
void Txtask3(void *p)			// task which writes data on to the Queue
{

	while(1)
	{
					  
		if(xQueueSend(xQueue,tx3,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
		{ // vTaskResume("RxTask");
		//Data added to Q	 
		}				  
		else
...
void Txtask4(void *p)			// task which writes data on to the Queue
{

	while(1)
	{
					  
		if(xQueueSend(xQueue,tx4,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
		{  //vTaskResume("RxTask");
		//Data added to Q
		}				  
		else
...
{  	 
 Init_UART0();
	...
	xTaskCreate(Txtask1,"TxTask 1", configMINIMAL_STACK_SIZE,NULL, tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(Txtask2,"TxTask 2", configMINIMAL_STACK_SIZE,NULL, tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(Txtask3,"TxTask 3", configMINIMAL_STACK_SIZE,NULL, tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(Txtask4,"TxTask 4", configMINIMAL_STACK_SIZE,NULL, tskIDLE_PRIORITY + 1, NULL);	
