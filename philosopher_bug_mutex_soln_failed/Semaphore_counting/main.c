...
Problem:To Simulate dining Philosopher's problem on RTOS
Concepts covered:	
Counting semaphores definition, creation and usage.
...
xSemaphoreCreateCounting( 5, 5 )
	1. 1st parameter mentions the available number of resources 
	2. 2nd parameter mentions the initial count of resources																

Note:make changes in config file acordingly
*/
//****************************************************************************************************/
...
{
			int i; 											//For ith Fork
		    char* str;			//Pointer for the passed Parameter
				...
				if(( xSemaphoreTake( xSemaphore, 1000 ) == pdTRUE )&&(forks_avail[i]==0))	
			{
					forks_avail[i]=1;
				 	 ...
				 	 if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
						{					
								if(xQueueSend(xQueue,s1,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
								{  
								if(xQueueSend(xQueue,str,1000) == pdTRUE)
								vTaskResume("RxTask");
								}
		 		  xSemaphoreGive( xMutex );
						...
						if(( xSemaphoreTake( xSemaphore, 2000 ) == pdTRUE )&&(forks_avail[(i+1)%5]==0))
				  				   { //Waits for 2000 ticcks for Left fork to be available
											...
											if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
												 {
												 if(xQueueSend(xQueue,s2,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
													{ 	if(xQueueSend(xQueue,str,1000) == pdTRUE)
															vTaskResume("RxTask");
													}
		 										  xSemaphoreGive( xMutex );
							 ...
							 if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
							   		{				
										if(xQueueSend(xQueue,s3,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
										{  
												if(xQueueSend(xQueue,str,1000) == pdTRUE)
												vTaskResume("RxTask");
										}
		 		 						 xSemaphoreGive( xMutex );
											...
											forks_avail[i]=0;	 
										 	forks_avail[(i+1)%5]=0;
												 	 ...
												 	 if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
							   		{				
										if(xQueueSend(xQueue,s4,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
										{  
												if(xQueueSend(xQueue,str,1000) == pdTRUE)
												vTaskResume("RxTask");
										}
								...
								if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
							   		{				
										if(xQueueSend(xQueue,s5,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
										{  
												if(xQueueSend(xQueue,str,1000) == pdTRUE)
												vTaskResume("RxTask");
										}
				   ...
				   {		 	 if(xSemaphoreTake(xMutex,portMAX_DELAY) == pdTRUE )
							   		{				
										if(xQueueSend(xQueue,s6,1000) == pdTRUE)   	// wait for 1000ms to tx queue message
										{  
												if(xQueueSend(xQueue,str,1000) == pdTRUE)
												vTaskResume("RxTask");
										}
